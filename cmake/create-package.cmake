...
FUNCTION(INDENTED_STATUS_MESSAGE str)
  STRING(REPEAT " " ${PACKAGE_MESSAGE_INDENT} INDENT_STRING)
  MESSAGE(STATUS "${INDENT_STRING}${str}")
ENDFUNCTION()

MACRO(INITIALIZE_DEPENDENCY name)
  FIND_PACKAGE(${name} QUIET)
  IF("${name}_FOUND")
    INDENTED_STATUS_MESSAGE("Found system package for ${name}")
  ELSE()
    IF(TARGET ${name})
      INDENTED_STATUS_MESSAGE("Found inherited target ${name}")
    ELSE()
      INDENTED_STATUS_MESSAGE("Initializing ${name} in git submodule")
...
MACRO(PACKAGE_MESSAGE var)
  INDENTED_STATUS_MESSAGE("${var}: ${PACKAGE_${var}}")
ENDMACRO()

MACRO(CREATE_HEADER_ONLY_PACKAGE)
  
  IF (NOT DEFINED PACKAGE_MESSAGE_INDENT)
    SET(PACKAGE_MESSAGE_INDENT 0)
  ELSE()
    MATH(EXPR PACKAGE_MESSAGE_INDENT "${PACKAGE_MESSAGE_INDENT} + 4")
  ENDIF()
  ...
  INDENTED_STATUS_MESSAGE("${PACKAGE_TITLE_STRING_SURROUND}")
  INDENTED_STATUS_MESSAGE("${PACKAGE_TITLE_STRING}")
  INDENTED_STATUS_MESSAGE("${PACKAGE_TITLE_STRING_SURROUND}")
  ...
  IF(DEFINED PACKAGE_DEPENDENCIES)
    INDENTED_STATUS_MESSAGE("Initializing dependencies")
  ...
  ELSE()
    ...
    INDENTED_STATUS_MESSAGE("File config.cmake.in not found in the cmake directory. Generating a generic version instead.")
  ENDIF()
  ...
  INDENTED_STATUS_MESSAGE("${PACKAGE_FOOTER_STRING}")
